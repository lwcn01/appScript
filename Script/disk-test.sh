...
function create_testfile_random1()
{
    echo "Creating source file(random 25M) $infile..."
    dd if=/dev/block/system of="$infile" bs=1024 count=25600
    sync
}

function create_testfile_random2()
{
    echo "Creating source file(random 2k) $infile..."
    dd if=/dev/block/system of="$infile" bs=1024 count=2
    sync
}

function create_patternfile_1()
{
...
function do_test()
{
    ...
    do
        ...
        while [ 1 ]
        do
            condition=$(df | sed  "s/  */ /g" | grep "data" | cut -d ' ' -f 4)
            echo $condition | grep -q "M"
            if [ "$?" -eq 0 ] 
            then
              echo "free size: $condition"
              if [ $(expr $(echo $condition | sed 's/.[0-9][A-Z]//g') + 0)  -le 25 ]
              then
